...
eventer( messageEvent, function(e) {
	...
	/**
			 *
			 * Resize iframe - since 1.2.0
			 *
			 */
			var pppIframe = document.getElementsByTagName("iframe")[0];			
			function resizeIframe() {
				var currentIframeHeight = pppIframe.scrollHeight;
				document.getElementById("ppplus").style.height = currentIframeHeight + "px";
				document.getElementsByTagName("iframe")[0].style.height = currentIframeHeight + "px";
			}
			function resizeIframeOnError() {
				var currentIframeHeight = pppIframe.scrollHeight + 250;
				if ( currentIframeHeight <= 710 ) {
					document.getElementById("ppplus").style.height = currentIframeHeight + "px";
					document.getElementsByTagName("iframe")[0].style.height = currentIframeHeight + "px";
				}
			}
	...
	if ( message["result"] == "error" ) {
		...
		document.getElementsByTagName("iframe")[0].style.height = "470px";
	...
	else if ( action == "resizeHeightOfTheIframe" ) {
	
		document.getElementById("ppplus").style.height = message["height"] + "px";
		document.getElementsByTagName("iframe")[0].style.height = message["height"] + "px";
...
if (typeof message['cause'] !== 'undefined') { //iFrame error handling
            ...
            switch (ppplusError)

                {
					...
					alert("Devido a um erro retornado, você será direcionado para a página de checkout do PayPal para finalizar o pagamento.");
